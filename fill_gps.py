"""
The code for handling missing GPS data in Oracle database. 

You can call "python fill_gps.py" and then follow the pop-up window to choose 
the txt file that contains a bunch of CSV file path (just like the uploading F25 in batch). 

Please note that there is a naming convention for the CSV file:
    The name of the CSV file has to match the name of the F25 with " gps" appended. 
    For example, the F25 filename is "XXX.F25", then the gps csv file should be "XXX gps.csv". 
    Note that there is a space between "XXX" and "gps".
"""
