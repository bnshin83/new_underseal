...
def process_file(input_file_path):
    # Replace spaces in the input file path with underscores to handle spaces in file names
    input_file_path = input_file_path.replace(' ', '_')

    output_file_path = os.path.join(
        os.path.dirname(input_file_path),
        os.path.basename(input_file_path).replace('.f25', '_LTE_calculations.csv', 1).replace('.F25', '_LTE_calculations.csv', 1)
    )
